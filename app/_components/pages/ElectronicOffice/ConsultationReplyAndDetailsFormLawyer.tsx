...
import { getListReservedFromDigitalGuide, replyTpConsultationFromLawyer } from "@/app/_api/queries/office.query";
...
const ConsultationReplyAndDetailsLawyer: React.FC<PropsIN> = ({ consultID }) => {
    ...
    const [digitalsReservations, setDigitalsReservations] = useState<any[]>([]);
    const [reservation, setReservation] = useState<any | null>(null);


    const { mutate: fetchReservationsDigitalGuide } = useMutation({
        ...
        onSuccess: (res: any) => {
            if (res.status === 200) {
                setDigitalsReservations(res.data.data.reservations);
    ...
    const { mutate: replyReservationsFromDigitalGuide } = useMutation({
    ...
    const handleSubmit = (event: FormEvent) => {
        ...
        replyReservationsFromDigitalGuide(formData);
    };
    if (loading) return (
        <motion.div
